    ...
    setState(() {
      enemyList = monsters
          .where((element) =>
              ...
              element.locations!.every((element) => element is String) &&
              element.locations!.isNotEmpty)
          .toList();
    });
  }

  void _getEnemyMapLocations(int index) {
    if (index == currentSelectedEnemy) {
      // remove locations
...
class ImageMapWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ImageMap(
        ...
        onTap: (region) {},
        regions: List<ImageMapRegion>.generate(
            locations.length, (index) => _fromMapToImage(locations[index])));
  }
}
...
class EnemyListWidget extends StatefulWidget {
  ...
  const EnemyListWidget(
      {super.key, required this.enemyList, required this.onEnemySelected});
...
class EnemyListWidgetState extends State<EnemyListWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return Align(
        ...
        child: Container(
            height: !_expanded ? screenSize.height * 0.13 : screenSize.height,
